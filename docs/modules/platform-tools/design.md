...
## 12. T0828-06 补充：真实字段映射与输入质量

本节在 2026-08-27 的真实 XDR 输入契约和告警关联链路设计基础上追加，不替换此前关于固定 JSONL、标准化、最小关联、风险研判、Mock 主链和安全边界的任何结论。

本日根据已取得的脱敏真实响应确认 `POST /api/xdr/v1/alerts/list`、`data.item`、`total/pageSize/page` 以及单条告警字段结构。真实响应通过现有 `NormalizedAlertRecord` 进入 `AlertRecord`，继续沿用既有 `AlertCorrelationService`、`RiskTriageService` 和 `Orchestrator`，不新增第二套上层告警对象。

`uuId` 优先作为稳定事件标识；`firstTime`、`lastTime`、`updateTime` 支持秒或毫秒 Unix 时间戳并转换为带东八区的时间；`srcIp`、`dstIp` 与端口数组取首个实体；`destination_ip` 优先作为受影响资产，缺失时才使用 `hostIp`；`devSourceName` 缺失时回退 `devUidDesc`，最终回退 `XDR`。真实数值严重度按受控规则映射，不把单一样本枚举擅自扩展为完整平台枚举。

分页仅使用页码模型，按 `total/page/pageSize` 判断是否继续；跨页按稳定提供方 ID 去重并保留字段更完整的记录。空结果、认证失败、网络超时、平台错误、非法 JSON、缺字段和非法值分别处理；缺少稳定 ID、事件时间或告警名称，以及端口越界时拒绝进入主链。所有真实凭据、真实响应值和敏感实体仍不得进入代码、日志、测试夹具、PR 或 GitHub。
...
为支持后续 T0828-07 的真实事件对照，本日补充 `tests/fixtures/xdr_contract/real_alert_records_sanitized.json`，其中同时提供一条脱敏的 `NormalizedAlertRecord` 和对应的 `AlertRecord`。两者使用同一事件标识、时间、目的资产、严重度和证据引用语义，所有实体均为占位值或 RFC 5737 文档地址，不是可用于查询的真实实体。

该示例沿用本设计既有的标准化与主链边界，可直接用于固定样例对照、字段差异记录和关联链路验证。
