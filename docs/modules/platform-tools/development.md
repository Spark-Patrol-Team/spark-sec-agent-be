...
## 12. T0828-06 今日任务反馈：真实字段映射与输入质量

本节是在 2026-08-27 已完成的真实 XDR 输入契约、字段映射准备和固定 JSONL 回归基础上的追加实现，既有历史实现与变更记录保持不变。

本日完成 `src/sec_agent/platforms/xdr_openapi.py` 的真实只读响应适配。真实 `POST /api/xdr/v1/alerts/list` 使用 `{page, pageSize}` 请求体，响应支持 `data.item`，并兼容已有 GET 测试夹具。`uuId`、`name`、`severity`、`srcIp`、`srcPort`、`dstIp`、`dstPort`、`hostIp`、`devSourceName`、`devUidDesc`、`firstTime`、`traceBackId` 等真实字段先转换为已有 `NormalizedAlertRecord`，再转换为已有 `AlertRecord`，下游仍使用原有主链。

适配器支持秒/毫秒 Unix 时间戳、IP/端口数组、数值严重度、目的资产回退和来源设备回退。分页按 `total/page/pageSize` 继续请求，跨页按稳定提供方 ID 去重，并保留字段更完整的重复记录。空结果返回 `empty_result`；缺少稳定标识、必填时间或告警名称、非法端口和非法数值返回 `field_mapping`，不会错误降级为固定样例。认证、超时、不可达和平台错误继续沿用既有结构化错误边界。

真实凭据、真实响应、真实 IP、事件 ID、资产 ID、详情 URL 和原始日志均未写入代码、日志、测试夹具或提交材料。
...
本日补充 `tests/fixtures/xdr_contract/real_alert_records_sanitized.json`，同时保存一条脱敏 `NormalizedAlertRecord` 和由其转换得到的 `AlertRecord`。示例严格复用现有模型字段和主链语义，便于陈敏直接对照字段映射、目的资产、严重度、事件时间和证据引用。

示例不包含真实告警 ID、真实 IP、真实资产 ID、Token、联动码、Cookie、原始响应或详情 URL；它仅表达真实响应中已确认的字段结构和转换关系，不可作为真实平台查询实体。
