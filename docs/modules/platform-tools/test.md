...
| 执行人 | 陈敏。 |
| 执行时间 | 2026-08-30。 |
| 基线分支与 Commit | `main` ；重放 PR #34 内容后复验。 |
...
| 2026-08-30 | `main` /PR#34 | 新增真实 XDR 字段映射与输入质量补充。 | 专项 13 passed；完整测试 143 passed, 1 skipped。 |
...
## 11. T0828-06 真实 XDR 字段映射与输入质量补充（2026-08-29）

本节在陈敏负责的 PR #22 历史测试记录基础上追加，不删除或改写此前内容。本轮只验证字段解析、标准化、输入质量、证据追溯、稳定 ID 去重和现有主链兼容性，不把 PR #28 中未经官方接线材料确认的鉴权实现作为测试依据。

脱敏结构覆盖 `data.item`、`uuId`、`firstTime/lastTime` 秒/毫秒时间戳、数值和文本严重度、IP/端口数组、`hostIp` 资产优先级、`dstIp` 候选、`devSourceName` 来源设备和 `traceBackId` 证据引用。重复记录按稳定 ID 去重并保留字段更完整版本；缺少稳定 ID、非法端口、非法严重度、非法状态和不合法列表结构必须失败，不得静默转换或冒充固定样例。

本地验证命令：

```text
PYTHONPATH=src python3 -m pytest -q tests/test_xdr_openapi_platform.py tests/test_t0828_xdr_field_mapping.py
13 passed
...
原有 PR #22 契约测试、固定 JSONL、告警关联、鉴权失败、超时、fallback、字段失败、AK/SK 和工具查询回归均保留。真实平台官方签名、真实凭据、HTTP 翻页和现场非空调用不在本地脱敏测试中冒充已完成。

## 12. 变更记录

| 日期 | 基线 | 新增内容 | 结果 |
|---|---|---|---|
| 2026-08-30 | 回归后最新 `main` | T0828-06 字段映射、输入质量、证据引用和稳定 ID 去重测试 | 专项 13 项通过；全量 143 项通过、1 项跳过 |
