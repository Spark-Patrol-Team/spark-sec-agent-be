...
| 任务/测试批次 | `T0828-06`｜真实字段映射与输入质量。 |
| 执行人 | 陈敏。 |
| 执行时间 | 2026-08-28。 |
| 基线分支与 Commit | `main` / PR #27 内容后复验。 |
...
本节在 2026-08-27 已有的契约测试、固定 JSONL 回归和主链验证记录之后追加，未删除或覆盖任何历史测试记录。

本日新增脱敏真实响应契约测试 `tests/test_xdr_real_contract.py`，覆盖真实 `data.item` 结构、`POST /api/xdr/v1/alerts/list` 请求体、`uuId` 稳定标识、数值严重度 70、Unix 秒级时间戳、源/目的 IP 与端口数组、目的资产规则、跨页去重以及重复记录字段完整度保留。

异常测试覆盖缺少可选字段、非法端口、缺少稳定标识、空数组、非法 JSON、认证失败、超时、平台错误和字段映射失败。空数组按 `empty_result` 处理，不生成虚假告警；字段转换异常按 `field_mapping` 处理，不错误降级到固定样例。

在 `APP_ENV=test` 下重新运行完整测试套件，结果为 **138 passed, 1 skipped**。既有固定 JSONL、`AlertRecord`、告警关联、调查编排、处置幂等和 API 回归测试均通过，确认真实字段转换没有破坏前端或后续流程。测试材料仅使用脱敏字段、占位符和文档地址，不包含真实凭据、真实 IP、真实事件 ID 或原始响应。
...
本日新增 `tests/test_xdr_real_record_fixture.py`，对 `tests/fixtures/xdr_contract/real_alert_records_sanitized.json` 中的 `NormalizedAlertRecord` 与 `AlertRecord` 分别执行现有 Pydantic 契约校验，并验证该 `AlertRecord` 能进入既有 `AlertCorrelationService` 主链。

测试确认两种对象的事件标识、时间、严重度和受影响资产一致，且能够生成包含资产实体和告警引用的 `SecurityEvent`。全量测试结果更新为 **140 passed, 1 skipped**。
