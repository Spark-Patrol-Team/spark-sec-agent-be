...
本文件记录处置闭环模块当前已经确认的回归范围，并区分以下层级：

- `L0`：只读查询、页面核对和本地结构化逻辑；
- `Mock`：本地有状态 Mock、幂等和独立验证；
- `平台记录`：平台侧是否留下执行、审批或审计痕迹；
- `设备生效`：真实设备状态是否发生变化；
- `外部阻塞`：真实平台动作、真实设备或授权条件尚未满足。

测试结论只在对应层级内成立，不能向上推导。
