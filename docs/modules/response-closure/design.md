...
gate_decision 与 response_evidence_scope 是不同字段。前者属于调查/知识侧门控信息，后者是主链处置授权边界；当前实现不把二者自动视为同义。knowledge refs 只是知识依据，不能替代原始处置证据；知识查询成功也不会自动升级 scope。
...
当前固定样例和 JSONL 样例的候选动作是 stateful_mock_containment，目标取调查结果中的第一个 affected_objects。动作建议不是授权；中、高、严重风险动作需要人工审批。
