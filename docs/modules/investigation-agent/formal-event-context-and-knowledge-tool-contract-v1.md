...
门禁为`out_of_scope`时，禁用知识工具并不足以保证最终报告不越界。正常LLM报告与fallback出口必须共用确定性清洗：

- `attack_chain`不得声称当前事件存在WebShell植入、持久化、后门、木马或最终载荷；对应英文变体同样禁止；
- `disposal_suggestions`不得给出排查/清除WebShell、查杀后门等当前场景专属动作；可保留日志、人工复核、转交匹配场景等中性建议；
- 提示词约束只是第一层，最终保证必须由报告解析代码完成；
- 清洗不得破坏第6节的事件证据确定性重建，也不得把知识来源重新写入事件证据。

该约束只证明仓库内自动化边界，不代表真实XDR/MCP/LLM平台报告已经完成复验。
