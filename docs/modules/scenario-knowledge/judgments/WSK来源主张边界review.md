...
### 7.1 WSK-011 来源错挂（✅ 已由沈洪旭在 PR#44 修复）

此前 WSK-011 引用 `Apache Knox Jira KNOX-2872`，其 “Webshell” 实为 Knox 网关自带的 Web 终端（非恶意后门），来源错挂。**沈洪旭已在 PR#44 将 WSK-011 来源替换为腾讯云安全指南**（`https://cloud.tencent.com/developer/article/2345678`，腾讯云 WebShell 排查指南，讲解检测工具失败时的替代排查方案），与“恶意 WebShell 工具查询失败”场景相符。本条已闭环。

### 7.2 部分知识卡来源 URL 缺失 / 不可在线核验（✅ 已补齐）

**2026-09-09 复核：15 张 WSK 卡均已提供来源 URL**（每卡至少一条 http(s) 链接），此前“空 URL”问题已由沈洪旭补齐。仍保留以下来源等级提醒（供判据收口用，非阻塞）：

- WSK-007 的 Idocdown、WSK-015 的 CSDN：属 **C 级**，只能作灵感，不能作已核实案例；
- WSK-008 的 IEEE 11513222：需权限/支付墙，无法读取正文，只能引用泛化结论；
- WSK-008 的 CoreRuleSet（GitCode）：可访问且为项目方一手分析（建议由 C 更正为 **B**），但属**单一项目观察**，不推广为普遍规律。
