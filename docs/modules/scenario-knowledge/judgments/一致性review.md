...
> 本文件是 T0905-03 交付物 #5/#6：把 10 份正式判据与陈敏 `PR#43` 门禁、沈洪旭 `PR#44` 的 case1-10 事件输入（`docs/modules/scenario-knowledge/knowledge-test-cases/caseN.json`）做一致性核验，并登记固定置信度/平台种子分等已知限制与 A/B 研判影响结论。**第 5.1-5.4 节为「预演」**（mock 门禁行为对照，留作过程留痕），**正式结论见第 5.5 节**（2026-09-09 基于杨景凡 `T0905-07` / PR#45 真实 A/B 完成）。
...
## 5. A/B 研判影响结论（5.1-5.4 为「预演」Rehearsal，正式结论见 5.5）

> ⚠️ **第 5.1-5.4 节为「预演」（Rehearsal）**：数据来自**确定性 mock 知识门禁行为对照**（非真实 LLM 逐案 `off/guarded` 风险分/置信度），仅用于先验证判据口径与知识增强的证据边界，**不**代表正式 A/B 结论。
>
> **正式结论见第 5.5 节**：2026-09-09 已基于杨景凡 `T0905-07` / PR#45 真实 A/B（10 案确定性 + 6 案端到端）完成正式 Review，重点核验知识增强有没有让“风险判断、置信度或结论”在**证据不足**的情况下变强（沿用 5.4 检查口径，把“不合理变化”登记为风险/缺陷）。
>
...
### 5.5 正式 A/B Review（T0905-07 / PR#45，2026-09-09）

> **本节省去「预演」标签，改为正式结论**：数据为杨景凡 `T0905-07` 真实 A/B——Agent 正式接入三档 `gate_decision`（`src/sec_agent/deep_agent/agent.py` 新增 `_bind_gate_decision`），并跑通 **确定性 10 案**（`scripts/ab_gatekeeper.py`，无需 LLM）+ **端到端 6 案**（`scripts/e2e_ab_gatekeeper.py`，真实 `agent.investigate()`，LLM 自主决策）。结论基于 PR#45 的 fork main head `0eb38cc`；本地已在 PR#45 工作树复现确定性对照，10 案逐案与回执一致。

**真实 A/B 证据链**

| 三档 gate_decision | 案例 | 确定性知识状态（guarded / off） | 端到端知识调用 / 报告知识引用（guarded / off） |
|---|---|---|---|
| in_scope | case9 | success(WSK-013) / not_registered | 4 次 / 7 条引用；off 未调用无引用 |
| weak_signal | case1-5、7、8 | partial（不回确认性知识）/ not_registered | 1-2 次 / 0 条（partial 拦截）；off 未调用无引用 |
| out_of_scope | case6、10 | failed(knowledge_scope_mismatch) / not_registered | case6 4 次 / 0 条（scope_mismatch）；case10 0 次（LLM 判非 WebShell 不查）；off 未调用无引用 |

**对照 5.4 检查口径的结论**

- **仅 `in_scope` 案真正使用知识**（case9，报告 7 条引用），与判据 `expected_scope` 一致。
- **证据不足/弱信号案（case1-5、7、8）知识被 `partial` 拦截，报告无任何确认性 WebShell 知识** → 未让结论“从可疑变确认”，未新增植入/执行/持久化/载荷/攻击确认。
- **域外/负向案（case6、10）知识被 `scope_mismatch` 拦截或不被调用** → 未把负向案解释成 WebShell，case10 由 LLM 自主判非 WebShell。
- **`off` 全部无知识**（工具不存在、无一调用），对照组成立。
- **未观察到“不合理变化”**：仅在开启知识时因门禁放行导致的风险等级无依据升高、置信度大幅提高、从“可疑”变“已确认”、新增输入中不存在的攻击事实、工具失败写成调查成功、生成更强处置建议——**均未出现**。

**风险登记更新**

| 原登记风险 | 本轮判读 | 状态 |
|---|---|---|
| §5.3 #1 无逐案真实风险分/置信度 | 本轮端到端已给出知识调用/引用并输出 `risk_level`；完整报告 JSON 为 `.gitignore` 本地产物，若需数值置信度 delta 可由杨景凡补充 | 基本解决（残留：数值 delta 未固化） |
| §5.3 #2 `fold_scope` fail-open（未知 strength 默认 CONFIRMED 放行知识） | 新集成不再使用 `fold_scope`；Agent 直接读 `gate_result.gate_decision.value`（三档），`_to_gate_decision` 对未确认/不确定一律落 `weak_signal`，**无 fail-open** | ✅ 已解决 |
| §5.3 #3 case7 良性档未单设 | 本轮 case7 实际也 `partial`（报告 0 引用），**行为安全**；但“MIXED/BENIGN 未单独设档”的口径问题仍保留给陈敏/杨嘉琪作为类目语义待办 | ⚠️ 行为已安全，口径待对齐 |

**收口与残留**

- ✅ 交付物 #6 正式 Review 完成：知识增强未在证据不足/域外案制造证据、未加强结论；仅在 `in_scope` 案正确使用知识。
- ⚠️ 残留：① 逐案 `off/guarded` 数值置信度 delta 未固化（全量报告 JSON 为本地产物）；② 知识增强对 `in_scope` 案的事实完整性提升仍以引用条数与抽检确认，未做统计校准（沿用第 4 节限制，不宣称百分比收益）。
...
- ✅ 一致性 Review：本文件
- ✅ 至少 6 案 A/B 研判影响结论（**预演**）：已登记（见第 5.1-5.4 节；8/10 案，含限制与缺陷点）
- ✅ 正式 A/B 最终 Review：已完成（见第 5.5 节；2026-09-09 基于杨景凡 `T0905-07` / PR#45 真实 A/B）。知识增强未在证据不足/域外案制造证据或加强结论；残留为逐案数值置信度 delta 未固化（`_knowledge_cases/` 本地产物，可再由杨景凡补齐）。
