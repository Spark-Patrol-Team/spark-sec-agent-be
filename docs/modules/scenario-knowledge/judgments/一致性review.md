...
>
> **正式结论见 `T0913-闫昱硕-正式ABReview与不通过项.md`**：此前“正式 A/B 已完成”的结论是在**未取得 actual 包**时形成的，已于 2026-09-13 删除（3.7 Review 第 4 项）。正式 Review 改为基于杨景凡 `T0905-07` 正式 10 案 guarded/off 运行产物的团队复核记录，见 T0913 第 2、3 节。本节 5.1-5.4 仅保留为**预演**过程留痕。
>
...
### 5.5 正式 A/B Review（已迁移）

> ⛔ **2026-09-13 更新（3.7 Review 第 4 项）**：本节此前的“正式 A/B 已完成”结论，是在**未取得 actual 运行包**的情况下形成的，已删除，不再作为交付物 #6 的正式结论。
>
> 正式 A/B Review 已迁移至 `T0913-闫昱硕-正式ABReview与不通过项.md`，基于杨景凡 `T0905-07` 正式 10 案 guarded/off 运行产物（head `0eb38cc`）的团队复核记录（肖迎春《T0905-05 正式 AB 结果处置边界复核》10 案矩阵）重做，并补 case6 越界判定、20 案 `need_manual_takeover=true` 解释与不通过项最小修改要求。
>
> 原始 20 份报告 JSON 为 `.gitignore` 运行产物、未入仓库；该证据边界已在 T0913 第 0 节明示。
...
- ✅ 判据绑定最终 case1-10 与 `WSK-*` 知识 ID：已在 10 份判据新增 `knowledge_ids_wsk`，并由 `test_wsk_binding_matches_final_knowledge_cards` 守护。
- ✅ 正式 A/B 最终 Review：**已迁移并在 2026-09-13 重做**，见 `T0913-闫昱硕-正式ABReview与不通过项.md`（10 案，基于杨景凡 `T0905-07` 正式产物复核记录）。旧“正式 A/B 已完成”结论已删除，原因见第 5.5 节说明。
- ⚠️ 已登记不通过项：case6 域外案报告文本越界风险、20 案 `need_manual_takeover` 全 true 无区分度、报告缺 `response_evidence_scope`/`ResponsePlan`/`disposition`；详见 T0913 第 7 节。
