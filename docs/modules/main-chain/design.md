...
| 以最新main合同为上游叠加评测接口 | PR #50及后续#58/#59/#60已进入主干，门禁、域外报告和响应范围必须由评测接口继续遵守 | 未从旧分支覆盖主干合同；先合入最新main，再保留`/eval/comparisons`、证据分层和Bridge新增字段 |
...
| 结果包提供运行Commit但未提供逐案例耗时 | 接口可绑定运行Commit、代码基线、模型和关键配置；耗时对比仍不可用 | 从`运行元数据与脱敏摘要.md`读取可公开元数据，`duration_ms`继续保持0并明确限制 |
| 人工Review未结构化加载时不输出优胜结论 | 不阻塞事实汇总；`guarded_wins/off_wins`只在结构化Review或确定性违规证据存在时成立 | 支持结果包内`_human_review.json`或`EVAL_COMPARISON_REVIEW_PATH`；缺失时保持`pending/TIE` |
| 总体能力证据与逐案胜负分开记录 | 本批GUARDED知识门禁行为10/10符合预期，并在明确适用案例增强知识依据和引用可追溯性；旧结果包的最终报告质量仅小幅变化 | 能力证据进入验收材料；逐案胜场仅由可追溯Review驱动，不把边界通过率或引用增加伪装成准确率 |
...
| 2026-09-14 | 当前工作区更新 | 以`main@24fd76e`为上游完成PR #49增量集成；保留最新门禁、域外报告和响应合同，评测转换不再以知识命中自动宣布优胜 | 是 |
