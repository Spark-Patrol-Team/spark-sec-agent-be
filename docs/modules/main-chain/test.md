...
|---|---|
| 模块 | 主链 |
| 任务/测试批次 | 真实 XDR 告警输入接入后主链回归；Bridge 显式装配优化；评测对比接口接入与正式包读取 |
| 执行人 | 李雨妍|
| 执行时间 | 2026-08-30；2026-08-31；2026-09-06；2026-09-07；2026-09-09 |
| 基线分支与Commit | 当前工作区，包含真实 XDR 告警接入、签名实现、主链文档补充、Bridge 显式装配和评测对比接口正式包读取 |
...
|---|---|---:|---:|---:|---|
| 主链测试通过率 | 本文列出的正式用例 Pass 数 / 正式用例总数 | 17 | 17 | 100% | EVID-MAIN-001 至 EVID-MAIN-017 |
| 自动化测试通过情况 | pytest 通过数 / pytest 已执行测试数 | 212 | 212 | 100% | EVID-MAIN-001 |
...
| 证据 | 位置 | 脱敏状态 | 支持的结论 |
|---|---|---|---|
| EVID-MAIN-001 | 本地命令输出：`uv run pytest -q`；结果 `212 passed in 35.92s` | 不含敏感信息 | 完整测试回归通过 |
...
- 本轮仍不能确认：真实深信服 MCP 工具、XDR 日志查询真实接口、真实 LLM 调查闭环、真实高风险处置动作、MySQL 真实环境持久化、FastGPT / 远程 Agent Bridge 实现，以及杨景凡最终正式 A/B 结果包内容。
