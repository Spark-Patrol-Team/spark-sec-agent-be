...
|---|---|
| 模块 | 主链 |
| 任务/测试批次 | 真实 XDR 告警输入接入后主链回归；Bridge 显式装配优化；评测对比接口接入 |
| 执行人 | 李雨妍|
| 执行时间 | 2026-08-30；2026-08-31；2026-09-06；2026-09-07 |
| 基线分支与Commit | 当前工作区，包含真实 XDR 告警接入、签名实现、主链文档补充、Bridge 显式装配和评测对比接口 |
...
- 正式 OFF/GUARDED 评测结果入库或动态读取，原因是当前 `/eval/comparisons` 使用后端固定 Mock 数据。
...
|---|---|---|---|---|---|---|---|---|---|
| MAIN-001 | P0 | 回归 | 执行完整 `pytest` | 全部测试通过 | 2026-09-07 复验 `210 passed in 45.15s` | Pass | 无 | EVID-MAIN-001 | 无 |
...
| MAIN-015 | P0 | 接口/评测 | `GET /eval/comparisons` | 返回 OFF/GUARDED 对比数据，包含 `schema_version`、`comparison_id`、`data_source`、`suite`、`summary`、`results` | 返回 `data_source=mock_fixture`、`suite.baseline=OFF`、`suite.candidate=GUARDED`；接口与 OpenAPI 回归 `8 passed in 0.53s` | Pass | 无 | EVID-MAIN-015 | 无 |
...
|---|---|---:|---:|---:|---|
| 主链测试通过率 | 本文列出的正式用例 Pass 数 / 正式用例总数 | 15 | 15 | 100% | EVID-MAIN-001 至 EVID-MAIN-015 |
| 自动化测试通过情况 | pytest 通过数 / pytest 已执行测试数 | 210 | 210 | 100% | EVID-MAIN-001 |
...
| 证据 | 位置 | 脱敏状态 | 支持的结论 |
|---|---|---|---|
| EVID-MAIN-001 | 本地命令输出：`uv run pytest -q`；结果 `210 passed in 45.15s` | 不含敏感信息 | 完整测试回归通过 |
...
| EVID-MAIN-015 | 本地命令输出：`uv run pytest tests/test_api_http.py tests/test_openapi_generation.py -q`；结果 `8 passed in 0.53s`；OpenAPI 生成输出 `接口数量: 9` | 不含敏感信息 | `GET /eval/comparisons` 可返回 OFF/GUARDED Mock 对比数据，并已进入 OpenAPI |
...
| `/eval/comparisons` 当前为 Mock 数据 | 期望返回正式 OFF/GUARDED 评测结果 | 不影响前端页面先行对接；影响正式验收数据真实性 | 正式 fixture 稳定后替换 Mock 数据源，并补数据源契约测试 |
...
- 本轮可确认：`GET /eval/comparisons` 已可用，返回 OFF/GUARDED 对比 Mock 数据，并已进入 OpenAPI。
