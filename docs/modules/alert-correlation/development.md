...
| `tests/test_event_field_signal_contract.py` | `TestContractVersionAndFields`（6）+ `TestGateFieldProvenance`（6，含字段面 13/白名单 10 一致性 4 条）+ `TestEvidencePairingById`（2） | 2026-09-13 新增：验证主链按《事件字段—来源—信号强度合同》v1.1 理解字段（`event_type` 机器契约、`alert_summaries`/`evidence_summaries` 按引用 ID 建映射、证据 ID 与摘要按 ID 对齐），并机械校验 `SecurityEventInput` 13 字段与门禁白名单 10 字段的差异面（`summary` 两侧都不在）。 |
| `docs/modules/alert-correlation/event-field-signal-contract.md` | 事件字段—来源—信号强度合同 v1.1 | 2026-09-13 新增：登记字段来源、信号来源与强度、三档判定组合、空/缺失/冲突处理；D1 已关闭，D2—D4 待冻结方确认。 |
...
| 2026-09-13 | `codex/gate-boundary-tests-and-field-contract`（基线 `main@0001bbd`，PR#50 已合入） | **补充门禁边界测试并登记字段—来源—信号强度合同 v1.1**：① 新增 `tests/test_gatekeeper_boundary.py` 24 条确定性用例（否定语义/单一通用进程/普通反序列化/内核驱动/大小写/空字段/冲突字段）；② 新增 `tests/test_event_field_signal_contract.py` 验证主链字段语义（14 条，含 13/10 字段面一致性）；③ 登记合同文档 `event-field-signal-contract.md`（D1 已按 `38cee87` 的引用ID映射实现关闭，D2—D4 待杨嘉琪冻结确认）。 | 边界测试暴露 3 类真实问题并已最小修复：`cmd.exe` 等通用进程名不再单独判 `in_scope`、内核/驱动证据转 `out_of_scope`、“排除/并非”类否定语义纳入否定范围。专项 24 passed；合同 14 passed（v1.0 时 2 条 xfail 已转通过）；本模块 6 个回归文件 63 passed；全量 340 passed, 1 skipped；case1-10 判定与固定样例主链结果均无回归。 |
