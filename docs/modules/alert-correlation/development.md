...
| 2026-09-06（陈敏今日新增：WebShell 门禁专项落地） | 基线 `feat/case-quality-check-and-tests` | **新增 WebShell 门禁系统（确定性逻辑，不使用 LLM）**：① 新增 `src/sec_agent/services/gatekeeper.py`（`WebShellGatekeeper`、`GatekeeperResult`、6 级 `SignalStrength`、4 类 `SignalSource`、9 字段白名单 `GATEKEEPER_WHITELIST_FIELDS`）；② 严格门禁读取边界：仅允许读取 `SecurityEventInput` 9 字段（event_id/event_type/severity/timestamp/source_ip/target_ip/alerts/evidence/initial_verdict），禁止读取 confidence、triage、trace_id、run_id，不得从知识库反向补入证据；③ 证据侧聚合比 event_type 标签更可靠，覆盖 case10（SSH 暴力破解虽 event_type=WebShell，但证据侧全域外，最终判 OUT_OF_SCOPE）；④ WebShell 专项升级：`IN_SCOPE_CONFIRMED`→ CRITICAL/95，`IN_SCOPE_WEAK`→ HIGH/75，`OUT_OF_SCOPE/BENIGN`→ 不升级；⑤ 三档门禁接口（OUT 档 1 / WEAK 档 2 / CONFIRMED 档 3）供杨嘉琪绑定 knowledge_query 与主链流转；⑥ 桥接合同对齐 `DeepAgentBridge._to_deep_agent_input` 与 10 个案例输入字段，交李雨妍确认。 | 新增 `tests/test_gatekeeper_case1_10.py` 专项测试 71 条全部通过；结合 `test_t0903_06_contract_regression.py`、`test_signal_extraction.py`、`test_tool_contract.py`、`test_packaging.py` 合计 115 passed，0 failed，0 regression。 |
