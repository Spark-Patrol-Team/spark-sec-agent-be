...
| 2026-09-06（陈敏今日新增） | 基线 `feat/case-quality-check-and-tests` | **WebShell 门禁模块设计落地**（位于 `services/gatekeeper.py`，复用 `SecurityEventInput` 合同）：① 输入边界（白名单）设计：严格只读 9 字段 `{event_id,event_type,severity,timestamp,source_ip,target_ip,alerts,evidence,initial_verdict}`，禁读 `confidence/triage/trace_id/run_id`；证据信号 description 必须等于输入 evidence 原文，禁止从知识库补伪证据。② 6 级分类体系：`OUT_OF_SCOPE / IN_SCOPE_CONFIRMED / IN_SCOPE_WEAK / MIXED / BENIGN_LIKE / INDETERMINATE`；聚合原则：`alerts+evidence` 证据侧比 `event_type` 标签更可靠（case10 SSH 暴力破解证据侧全域外，覆盖 event_type=WebShell，最终判 OUT_OF_SCOPE，避免标签误分类直通）。③ 4 类信号来源必带标注：`event_type/alerts/evidence/triage`（注：`alerts` 与 `evidence` 均指从原始安全事件中检索到的、导致告警触发的证据字段，而非 Agent 的调查输出），每条 `GatekeeperSignal` 都有 source 字段。④ WebShell 专项升级：IN_SCOPE_CONFIRMED→CRITICAL/95；IN_SCOPE_WEAK→HIGH/75；OUT_OF_SCOPE/BENIGN→不升级。⑤ 三档门禁 × knowledge_query 绑定设计：档 1 OUT（不触发调查/knowledge_query，仅转交攻击链）；档 2 WEAK（进弱信号 knowledge_query，产出调查清单/误报条件/证据缺口，禁止直接判真）；档 3 CONFIRMED（升级 CRITICAL 95，进入原理/攻击链/IOC/处置建议 knowledge_query + 深度调查主链）。⑥ GatekeeperResult 输出合同：`allowed/forbidden_fields / signals[] / overall_strength / upgraded_severity,score / checklist,false_positive_conditions,evidence_gaps / input_quality_issues`。⑦ 15 分钟关联窗口对齐：调查清单中默认写入「查询目标 IP 近 15 分钟 HTTP 访问日志」，与现有关联服务一致。⑧ 三方交接职责：杨嘉琪按⑤档绑定主链与 knowledge_query；闫昱硕按 10 案信号强度表复核词典与聚合，必要时通过增补 case 调整；李雨妍核对 CLI/bridge/主链三处入口都用 `SecurityEventInput.from_dict(...)` 合同，确保字段一致。 | 是。`test_gatekeeper_case1_10.py` 71 条通过，115 条组合回归无回归，幂等稳定可复现。 |
