...
| 2026-09-13 | `codex/gate-boundary-tests-and-field-contract` | 关联模块输出契约扩展为 `SecurityEvent.event_type / alert_summaries / evidence_summaries`（机器契约字段，不由 `summary` 反推）；新增《事件字段—来源—信号强度合同》v1.0 登记本模块字段来源与下游门禁的判定语义。边界测试暴露并修复 3 类误判：通用进程名单独出现不再判 `in_scope`、内核/驱动证据转 `out_of_scope`、“排除/并非”类否定语义纳入否定范围。 | 是。`tests/test_gatekeeper_boundary.py` 24 passed；`tests/test_event_field_signal_contract.py` 6 passed + 2 xfail（合同 D1 证据配对待冻结）；全量 331 passed, 1 skipped。 |
