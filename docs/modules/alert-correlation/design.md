...
| 2026-09-13 | `codex/gate-boundary-tests-and-field-contract`（基线 `main@0001bbd`，PR#50 已合入） | 登记《事件字段—来源—信号强度合同》**v1.1**：记录 `event_type` / `alert_summaries` / `evidence_summaries` 的来源与“按引用 ID 建映射”的对应关系（D1 已由 `38cee87` 实现并关闭）；并明确 `summary` 既不进入 `SecurityEventInput`（13 字段）也不在门禁白名单（10 字段），门禁不可读。门禁边界测试暴露并修复 3 类误判：通用进程名单独出现不再判 `in_scope`、内核/驱动证据转 `out_of_scope`、“排除/并非”类否定语义纳入否定范围。 | 是。`tests/test_gatekeeper_boundary.py` 24 passed；`tests/test_event_field_signal_contract.py` 14 passed（含字段面/白名单一致性 4 条；D1 由 xfail 转通过）；`tests/test_gatekeeper_case1_10.py` 75 passed；全量 340 passed, 1 skipped。 |
