...
| 2026-09-13 | `codex/gate-boundary-tests-and-field-contract` | 新增门禁边界回归 `tests/test_gatekeeper_boundary.py`（24 条，7 类：否定语义 / 单一通用进程 / 普通反序列化 / 内核驱动 / 大小写 / 空字段 / 冲突字段）与合同一致性测试 `tests/test_event_field_signal_contract.py`（6 条通过 + 2 条记录合同 §5 D1 的 xfail）。 | 边界测试首轮 6 failed → 最小修复门禁关键词语义后 **24 passed**；`tests/test_gatekeeper_case1_10.py` 75 passed 无回归；本模块 6 个回归文件 63 passed；全量 331 passed, 1 skipped。固定样例主链仍为 `in_scope`，case1-10 判定与修复前一致。 |
