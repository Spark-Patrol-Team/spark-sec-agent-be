...
| 2026-09-13 | `codex/gate-boundary-tests-and-field-contract`（基线 `main@0001bbd`） | 新增门禁边界回归 `tests/test_gatekeeper_boundary.py`（24 条，7 类：否定语义 / 单一通用进程 / 普通反序列化 / 内核驱动 / 大小写 / 空字段 / 冲突字段）与合同一致性测试 `tests/test_event_field_signal_contract.py`（14 条，合同 v1.1；含 13 字段输入面 / 10 字段门禁白名单一致性，`summary` 两侧都不在；D1 证据配对已按引用ID映射实现，v1.0 的 2 条 xfail 转为通过）。 | 边界测试首轮 6 failed → 最小修复门禁关键词语义后 **24 passed**；合同 **14 passed**；`tests/test_gatekeeper_case1_10.py` 75 passed 无回归；本模块 6 个回归文件 63 passed；全量 340 passed, 1 skipped。固定样例主链仍为 `in_scope`，case1-10 判定与修复前一致。 |
