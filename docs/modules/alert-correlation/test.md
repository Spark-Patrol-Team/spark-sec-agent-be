...
| 2026-09-06（陈敏今日新增：WebShell 门禁专项自动化） | 基线 `feat/case-quality-check-and-tests` | 新增专项测试 `tests/test_gatekeeper_case1_10.py`，覆盖 6 大任务，共 71 条断言。运行方式：`$env:PYTHONPATH="src"; python -m pytest tests/test_gatekeeper_case1_10.py -v`。覆盖矩阵：① Task1 字段白名单 4 条；② Task2 信号来源与不可伪造性 3 条；③ Task3 case1-10 输入质量与负向约束 28 条（含 case6 纯非 WS、case7 合法 Base64、case8 弱文件名、case9 强组合、case10 SSH 域外 5 条专项）；④ Task4 逐案信号标注 10 条；⑤ Task5 升级规则（95/75/0）3 条；⑥ Task6 三方交接断言 3 条（杨嘉琪-三档门禁绑定、闫昱硕-信号强弱与判据、李雨妍-合同对齐）。稳定性：全部为确定性规则（关键词/集合运算/正则），不调用任何 LLM；反复运行幂等 71 passed。 | 本地实跑 **71 passed 0 failed**。与既有 `test_t0903_06_contract_regression.py`（21）、`test_signal_extraction.py`（17）、`test_tool_contract.py`（5）、`test_packaging.py`（1）合计 115 passed 0 failed，**零回归**。GO/NO-GO 结论：**GO**（已达到门禁自动化基线）。 |
