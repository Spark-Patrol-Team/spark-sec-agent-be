...
class ResponseBoundaryTest(unittest.TestCase):
    ...
    def test_scope_resolver_does_not_reclassify_domain_from_report_prose(self) -> None:
        report = self._confirmed_webshell_report(
    ...
    def test_scope_resolver_single_cmd_process_is_only_weak_signal(self) -> None:
        event = self._event(
            event_type="webshell",
            evidence_summaries={"evidence-proc-001": "检测到子进程 cmd.exe 启动"},
        )
        scope = ResponseEvidenceScopeResolver().resolve(
            self._confirmed_webshell_report(),
            self._triage(response_evidence_scope=None),
    ...
    def test_scope_resolver_single_w3wp_process_is_only_weak_signal(self) -> None:
        event = self._event(
            event_type="webshell",
            evidence_summaries={"evidence-proc-001": "检测到 w3wp.exe 工作进程运行"},
        )
        scope = ResponseEvidenceScopeResolver().resolve(
            self._confirmed_webshell_report(),
            self._triage(response_evidence_scope=None),
    ...
    def test_scope_resolver_conflicting_structured_signals_fail_closed(self) -> None:
        event = self._event(
            event_type="sql_injection",
    ...
    def test_scope_resolver_uses_structured_evidence_summary_for_domain_boundary(self) -> None:
        event = self._event(
            event_type="webshell",
            evidence_summaries={"evidence-http-001": "SSH 暴力破解登录失败，无 WebShell 行为"},
        )
        scope = ResponseEvidenceScopeResolver().resolve(
            self._confirmed_webshell_report(),
            self._triage(response_evidence_scope=None),
        ...
        *,
        ...
        return SecurityEvent(
            ...
            if evidence_summaries is not None
                ...
                "evidence-http-001": "异常 POST 请求命中可疑 WebShell 文件",
                "evidence-proc-001": "IIS 工作进程 w3wp.exe 派生 cmd.exe，并调用 Process.Start",
            },
        )
