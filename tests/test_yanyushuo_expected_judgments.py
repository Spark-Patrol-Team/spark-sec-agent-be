...
KNOWLEDGE_FILE = REPO / "src" / "sec_agent" / "deep_agent" / "knowledge" / "webshell-knowledge.md"

KNOWN_SCOPES = {"in_scope", "weak_signal", "out_of_scope"}

# 冻结汇总 Schema 的 K-WEBSHELL-* 六组 → 最终 WSK-* 知识卡展开（见 WSK来源主张边界review.md 第 2 节）
WSK_EXPANSION = {
...
def _expand_group_ids(ids: list[str]) -> list[str]:
    out: list[str] = []
    for group in ids:
        for wsk in WSK_EXPANSION[group]:
            if wsk not in out:
                out.append(wsk)
    return out
...
def test_wsk_binding_matches_final_knowledge_cards() -> None:
    """判据绑定最终 WSK-* 知识卡：WSK ID 必须真实存在于唯一知识源，且与 K-WEBSHELL-* 组展开一致。"""
    from sec_agent.deep_agent.tools.knowledge import parse_knowledge_cards

    text = KNOWLEDGE_FILE.read_text(encoding="utf-8")
    card_ids = {card.knowledge_id for card in parse_knowledge_cards(text)}
    assert card_ids == {f"WSK-{n:03d}" for n in range(1, 16)}, (
        f"最终知识卡 ID 集合异常: {sorted(card_ids)}"
    )

    for i in range(1, 11):
        d = _load_expected(i)
        assert "knowledge_ids_wsk" in d, f"case{i} 缺少 knowledge_ids_wsk 绑定"
        w = d["knowledge_ids_wsk"]
        for key in ("allowed", "required", "forbidden"):
            assert isinstance(w[key], list), f"case{i} knowledge_ids_wsk.{key} 应为 list"
            for wsk in w[key]:
                assert wsk in card_ids, f"case{i} WSK ID {wsk} 不在最终知识卡中"
        assert set(w["forbidden"]).isdisjoint(set(w["required"])), (
            f"case{i} WSK forbidden 与 required 有交集"
        )
        assert set(w["required"]).issubset(set(w["allowed"])), f"case{i} WSK required 必须 ⊆ allowed"
        assert set(w["allowed"]) == set(_expand_group_ids(d["allowed_knowledge_ids"])), (
            f"case{i} WSK allowed 与 K-WEBSHELL 组展开不一致"
        )
        assert set(w["required"]) == set(_expand_group_ids(d["required_knowledge_ids"])), (
            f"case{i} WSK required 与 K-WEBSHELL 组展开不一致"
        )
        assert set(w["forbidden"]) == set(_expand_group_ids(d["forbidden_knowledge_ids"])), (
            f"case{i} WSK forbidden 与 K-WEBSHELL 组展开不一致"
        )
        if d["expected_scope"] == "out_of_scope":
            assert w["allowed"] == [] and w["required"] == [], f"case{i} OOS 不应允许/要求 WSK 知识"
            assert set(w["forbidden"]) == card_ids, f"case{i} OOS 应禁止全部 WSK 卡"
