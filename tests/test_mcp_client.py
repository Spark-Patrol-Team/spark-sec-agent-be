...
验证点（对齐 28 日真实平台联调的 dbproxy 工具契约）：
  ...
  4. 非 dbproxy 工具（无 code/data 结构，如 secgpt 研判）→ 按文本原样 success；
  5. 上述结果经 ToolResult.to_str() 后可被 LLM 消费，区分「空数据」与「失败」，
...
class TestNonDbproxyTool(unittest.TestCase):
    """非 dbproxy 工具（如 secgpt / 检测大模型）无 code/data 结构，按文本 success。"""
