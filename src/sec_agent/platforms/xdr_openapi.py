...
class XdrOpenApiAdapter:
    ...
    def _to_alert_record(self, raw: Mapping[str, Any]) -> AlertRecord:
        ...
        record = self._from_normalized(normalized)
        return self._with_raw_scenario_fields(record, raw)
    ...
    def _to_normalizer_raw(self, raw: Mapping[str, Any]) -> dict[str, Any]:
        ...
        event_time = self._resolve_event_time(raw)
        ...
        alert_grade = (
            self._first_text(raw, "alert_grade", "severity", "raw_severity", "alertGrade", "level")
            or "中危"
        )
        alert_classification = self._first_text(
            raw, "alert_classification", "category", "type", "ruleName", "attackStage"
        )
        source_ip = self._first_text(raw, "source_ip", "src_ip", "srcIp", "sourceIp", "src")
        destination_ip = self._first_text(
            raw, "destination_ip", "dst_ip", "dstIp", "destinationIp", "dst", "affected_asset"
        )
        ...
        return {
            ...
            "source_ip": source_ip,
            "source_port": raw.get("source_port", raw.get("src_port", raw.get("sourcePort"))),
            "destination_ip": destination_ip or host_ip,
            "destination_port": raw.get("destination_port", raw.get("dst_port", raw.get("destinationPort"))),
            "host_ip": host_ip,
    ...
    def _resolve_event_time(self, raw: Mapping[str, Any]) -> str | None:
        value = (
            raw.get("event_time")
            or raw.get("alert_time")
            or raw.get("occurred_at")
            or raw.get("time")
            or raw.get("created_at")
            or raw.get("firstTime")
            or raw.get("lastTime")
        ...
        if isinstance(value, str):
            ...
            if stripped and stripped.replace(".", "", 1).isdigit() and len(stripped) >= 10:
                return self._epoch_to_iso(float(stripped))
            if stripped:
                return stripped
        return None
    ...
    def _with_raw_scenario_fields(self, record: AlertRecord, raw: Mapping[str, Any]) -> AlertRecord:
        scenario = dict(record.scenario_fields)
        confidence = raw.get("confidence")
        if confidence is not None:
            try:
                scenario["confidence"] = int(confidence)
            except (TypeError, ValueError):
                pass
        gpt_desc = raw.get("gpt_result_desc")
        if gpt_desc:
            scenario["gpt_result_desc"] = str(gpt_desc)
        stage = self._first_text(raw, "attackStage", "attack_stage", "stage")
        if stage:
            scenario["stage"] = stage
        return record.model_copy(update={"scenario_fields": scenario}, deep=True)
