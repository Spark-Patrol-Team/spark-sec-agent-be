...
把《最小 WebShell 知识包》（沈洪旭维护的权威版）解析为15张结构化知识卡，Agent 在调查中
通过知识ID、主题或受控别名获取适用条件、必要证据、调查步骤、误报和处置边界等参考信息。
...
class KnowledgeQueryTool(Tool):
    ...
    description = (
        "检索内置 WebShell 知识包（等价于 knowledge.query）：按知识ID、主题或受控别名"
        "获取适用条件、必要证据、调查步骤、误报、禁止推断、来源和处置边界。"
        "关键词示例：WSK-001、WebShell攻击原理、WebShell证据检查清单、WebShell处置建议。"
    )
    parameters = {
        "type": "object",
        "properties": {
