...
class DeepInvestigationAgent:
    ...
    def _parse_report(self, content: str, event: SecurityEventInput, tool_records: list[dict]) -> InvestigationReport:
        ...
        # 1. 没有任何真实成功/部分成功的工具调用时，不接受 LLM 生成的调查步骤和结论。
        #    这样可避免 MCP 连接失败、超时、鉴权失败时，LLM 虚构“已调用工具”和“工具返回结果”。
        successful_records = [
        ...
        # 2. investigation_steps 只能引用代码侧真实执行过的工具。
        #    LLM 可以总结真实调用，但不能创造不存在的工具调用记录。
        executed_tools = {
            r.get("tool")
        ...
        data["investigation_steps"] = [
            ...
            if isinstance(step, dict) and step.get("tool") in executed_tools
        ]

        # 3. 用代码采集的真实工具调用记录覆盖 LLM 输出，保证最终报告可审计。
