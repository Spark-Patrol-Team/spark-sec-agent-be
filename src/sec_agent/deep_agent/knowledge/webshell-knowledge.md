> 口径说明：文中“5分钟、10秒、30分钟、前后24小时”等固定时间窗，以及“出站流量大小异常”等流量阈值，均为便于首轮筛查的工程启发式，不是权威来源给出的通用事实或硬判定阈值。实际运行必须结合资产基线、日志时钟偏差、采集延迟、流量基线（区分业务高峰与非业务时段）和业务场景调整，且不得仅凭命中时间窗或单一流量阈值确认攻击。
...
- 禁止推断：
  ...
  - 禁止将“植入方式分类（上传漏洞利用、后台功能滥用、SQL 注入导出、第三方代码预置后门等）”当作具体案例的既成结论：该分类仅为调查枚举/启发，需有对应日志或漏洞证据后才能用于个案
- 来源URL：
...
- 必要证据：
  ...
  - 出站流量大小异常（工程启发式：以该资产自身业务流量基线为准，无统一数值阈值；“明显大于正常业务请求响应”仅为筛查线索）
- 调查步骤：
...
- 必要证据：
  ...
  - 已确认受影响资产的范围和业务重要性
  - 证据强度分级（Level 0-4）为本卡的内部解释框架，须与门禁三档对齐：Level 0 → out_of_scope；Level 1-2（弱信号/证据不足）→ weak_signal；Level 3-4（强证据组合）→ in_scope，最终结论以门禁三档为准，避免多套分级并存
- 调查步骤：
